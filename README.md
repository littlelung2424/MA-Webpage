...
5. Confirm the Supabase table exists with the exact columns in the SQL above, re-run `supabase/migrations/20260604000000_add_intake_admin_fields.sql` so the `service_role` API role receives explicit table grants, then redeploy again after saving any Vercel storage or variable changes.
