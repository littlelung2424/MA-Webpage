...
1. Open `/api/admin/intake/debug` on the same deployment while signed in with admin Basic Auth. The endpoint does not return secret values; it reports the configured Supabase URL host/project ref, selected key variable/kind, readiness checks, and a one-row REST probe status/snippet.
...
5. Confirm the private Blob store is connected to this exact Vercel project and environment. If the store uses read-write tokens instead of automatic project authentication, confirm `BLOB_READ_WRITE_TOKEN` is present in that environment.
