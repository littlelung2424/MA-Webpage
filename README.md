...
The admin page reads submissions newest first through Supabase REST using the elevated server-only key. It updates only `status` and `internal_notes` through a server action. For modern `SUPABASE_SECRET_KEY=sb_secret_...` keys, requests send the secret key as the Supabase `apikey` only; for legacy JWT `SUPABASE_SERVICE_ROLE_KEY` values, requests also send `Authorization: Bearer <service-role-key>`. If the dashboard shows a Supabase 401/403, confirm `SUPABASE_SECRET_KEY` is a Supabase `sb_secret_...` key or `SUPABASE_SERVICE_ROLE_KEY` is the legacy `service_role` JWT for the same project as `SUPABASE_URL`; anon/public/publishable keys can insert only if policies allow them, but they cannot read the admin list when Row Level Security blocks selects.
